...
async function openDayFilterPanel(mode) {
  ...
  // 見出しはホームの検索・おまかせ画面と同じ表記(「検索」「おまかせ」)に揃える。
  dayFilterModeLabel.innerHTML = mode === 'search'
    ? '<svg class="heading-ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>検索'
    : '<svg class="heading-ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>おまかせ';
  // 検索モードのみ、ホームの検索画面と同じ「タグ/キーワード検索」の見た目にする(検索結果画面は独自のままでよい)。
  dayFilterFreeTagLabel.textContent = mode === 'search' ? 'タグ/キーワード検索' : 'タグ検索';
  dayFilterFreeTagInput.placeholder = mode === 'search' ? 'レシピ名の一部またはタグ名を入力' : 'タグ名で選択・入力';
